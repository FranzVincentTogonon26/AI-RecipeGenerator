...
class PantryItem {
    ...
    static async findByUserId( userId, filters = {} ){
        ...
        if(filters.is_running_low !== undefined){
            paramsCount++;
            query += ` AND is_running_low = $${paramsCount}`;
    ...
    static async getExipiringSoon( userId, days = 7 ){
        const result = await db.query(
            `SELECT * FROM pantry_items 
             ...
             ORDER BY expiry_date ASC`, [userId]
        );
        return result.rows;
    }
    ...
    static async update( id, userId, updates ){
        ...
        const result = await db.query(
            `UPDATE pantry_items
             ...
             RETURNING * `, [name, quantity, unit, category, expiry_date, is_running_low, id, userId]
        );
        return result.rows[0];
    }
    ...
    static async delete( id, user_id ){
        const result = await db.query(
            'DELETE FROM pantry_items WHERE id = $1 AND user_id = $2 RETURNING * ', [id , user_id]
        );
        return result.rows[0];
    }
    ...
    static async getStats( userId ){
        const result = await db.query(
            `SELECT 
             ...
             FROM pantry_items WHERE user_id = $1`, [userId]
        );
        return result.rows[0];
    }
