        ...
        try {
            ...
            const result = await client.query(
                `SELECT ri.ingredient_name, ri.unit, SUM(ri.quantity) as total_quantity
                 FROM meal_plans mp JOIN recipe_ingredients ri ON mp.recipe_id = ri.recipe_id
                 WHERE mp.user_id = $1 AND mp.meal_date >= $2 AND mp.meal_date < $3
                 GROUP BY ri.ingredient_name, ri.unit`, [userId, startDate, endDate]
            );

            const ingredients = result.rows;

            // get pantry item to subtract
            ...
            const pantryMap = new Map();
                  pantryResult.rows.forEach(item => {
                    const key = `${item.name.toLoweCase()}_${item.unit}`;
                                 pantryMap.set(key, item.quantity);
                  });
            
            // insert shopping list items ( subtract pantry quantities )
            for( const ing of ingredients ){
                const key = `${ing.ingredient_name.toLoweCase()}_${ing.unit}`;
                ...
                if(neededQty > 0){
                    await client.query(
                        `INSERT INTO shopping_list_items (user_id, ingredient_name, quantity, unit, from_meal_plan, category)
                         VALUES ($1, $2, $3, $4, true, $5)`, [userId, ing.ingredient_name, neededQty, ing.unit, 'Uncategorized']
                    );
                }
        ...
        const result = await db.query(
            `SELECT 
                ...
                json_agg(
                    json_build_object(
                        'id', id,
                        'ingredient_name', ingredents_name,
                        'quantity', quantity,
                        'unit', unit,
             ...
             WHERE user_id = $1 GROUP BY category ORDER BY category`, [userId]
        );
        return result.rows;
    }
    ...
    static async update( id, userId, updates ){
        ...
        const result = await db.query(
            `UPDATE shopping_list_items
             ...
             WHERE id = $6 AND user_id = $7 RETURNING * `, [ingredient_name, quantity, unit, category, is_checked, id, userId]
        );
        return result.rows[0];
    }
            ...
            for (const item of checkedItems.rows) {
                await client.query(
                    `INSERT INTO pantry_items (user_id, name, quantity, unit, category)
                     VALUES ($1, $2, $3, $4, $5)`, [userId, item.ingredient_name, item.quantity, item.unit, item.category]
                );    
            }
